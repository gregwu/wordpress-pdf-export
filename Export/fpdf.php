...
protected $flowingBlockAttr; //EDITEI - added
...
function SetLeftMargin($margin, $d)
{
	error_log("SetLeftMargin($margin, $d) \n", 3, "/tmp/error.log");
	// Set left margin
	$this->lMargin = $margin;
...
{
	error_log("AddPage($orientation), fun:". debug_backtrace()[1]['function']."\n", 3, "/tmp/error.log");
	// Start a new page
	if($this->state==3)
...
function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $d='')
{
	...
	error_log("Cell($d, ". debug_backtrace()[1]['function']. ", w:$w, h:$h, x:$this->x, y:$this->y, txt:$txt, border:$border, ln:$ln, align:$align, fill:$fill, link:$link)\n", 3, "/tmp/error.log");
	$this->lasth = $h;
	if($ln>0)
	{
		// Go to next line
		$this->y += $h;
		if($ln==1)
			$this->SetX($this->lMargin, 2);
	}
	else {
		$this->SetX($this->x + $w, 3);
	}
...
function getWMax(){
	...
	$wmax = ($w-2*$this->cMargin)*1000/$this->FontSize;
	if($w < 0){
		error_log("   XXXXX: w is less than 0 => $txt");
		$wmax = 27000;
	}
	return $wmax;
}
...
function Write($h, $txt, $link='', $isList=0)
{
	// Output text in flowing mode
	if(!isset($this->CurrentFont))
		$this->Error('No font has been set');
	$cw = &$this->CurrentFont['cw'];
	$this->w = 105; //Greg two column hack
	...
	while($i<$nb)
	{
		...
		{
			// Explicit line break
			$this->Cell($w,$h,substr($s,$j,$i-$j),0,2,'',false,$link, 1);
			$i++;
			$sep = -1;
			$j = $i;
			$l = 0;
		...
		if($l>$wmax)
		{
			...
			if($sep==-1)
			{
				if($this->x > $this->lMargin)
				{
					...
					$i++;
					$nl++;
					error_log("WWWWW: w: $w , wmax: $wmax,  i: $i, nl: $nl ");
					continue;
				}
				if($i==$j){
					$i++;
				...
				$this->Cell($w,$h,substr($s,$j,$i-$j),0,2,'',false,$link, 2);
			}
			else
			{
...
function Ln($h=null)
{
	error_log("Ln: $h ".debug_backtrace()[1]['function']." \n", 3, "/tmp/error.log");
...
function SetX($x, $d = '')
{
	error_log("SetX($x, $d) ". debug_backtrace()[1]['function']. " : ". debug_backtrace()[2]['function']."\n", 3, "/tmp/error.log");
	// Set x position
	if($x>=0)
		$this->x = $x;
	else
...
{
	error_log("SetY($y) ". debug_backtrace()[1]['function']. " : ". debug_backtrace()[2]['function']."\n", 3, "/tmp/error.log");
