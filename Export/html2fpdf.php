...
//! @return An object (a class instance)
  ...
  //$this->DisplayPreferences('');
	...
	$this->SetFont('courier','',11); // Changeable?(not yet...)
  ...
  $this->col = 0;
  $this->width = 92; // Greg: width of a column in the table
...
function AcceptPageBreak()
{
    ...
    error_log("\n\n\nAcceptPageBreak fun: ".debug_backtrace()[1]['function']. " col:".$this->col. " y0: ".$this->y0. "\n", 3, "/tmp/error.log");
   
    if($this->col == -1)
    {
      $this->SetCol($this->col+1);
      $this->SetY($this->y0 + 20, false);
      $this->SetLeftMargin(110, 'AcceptPageBreak col -1');
      return false;
    }
    else if($this->col == 0) 
    {
        ...
        $x = $this->x;
        $this->SetCol($this->col+1);
        $this->SetLeftMargin(10, 'AcceptPageBreak col 0');
        //$this->SetX($this->lMargin);
        // Set ordinate to top
        if($this->x > $x && $this->x > 106)
        {
          $this->SetY($this->y0 , false);
          $this->SetLeftMargin(110, 'AcceptPageBreak col 00');
        }
...
/* $e == content */
 if(empty($html)) return;
  ...
  if ($this->usecss) $html = $this->ReadCSS($html);
	...
	$html=str_replace(['<?', "\xe2\x80\xa8", "\xc2\xa0"],['< ', '<br>', ' '],$html); //Fix '<?XML' bug from HTML code generated by MS Word
...
error_log("OpenTag: ".$tag." ".print_r($attr,true)."\n", 3, "/tmp/error.log");
  ...
  $this->blockjustfinished=false;
    ...
    case 'ADDRESS': 
      ...
      {
        if (!empty(trim($this->textbuffer[0][0])))
        {
          $this->listitem[] = array($this->listlvl,$this->listnum,$this->textbuffer,$this->listoccur[$this->listlvl]);
          //error_log('ul::'.$this->listlvl.' >'.$this->listnum.' ='.$this->textbuffer[0][0].' :'.$this->listoccur[$this->listlvl]);
          $this->listnum++;
        }
  		  $this->textbuffer = array();
      ...
      else
      {
        ...
        {
          $this->listitem[] = array($this->listlvl,$this->listnum,$this->textbuffer,$this->listoccur[$this->listlvl]);
          //error_log('li::'.$this->listlvl.' >'.$this->listnum.' ='.$this->textbuffer[0][0].' :'.$this->listoccur[$this->listlvl]);
          $this->listnum++;
        }
  		  $this->textbuffer = array();
      }
      ...
      //in case of malformed HTML code. Example:(...)</div><li>Content</li><div>DIV1</div>(...)
  	  if ($this->listlvl > 0) // We are closing (omitted) OL/UL tag(s)
   	  {
	        $this->buffer_on = false;
          if (!empty(trim($this->textbuffer[0][0]))) $this->listitem[] = array($this->listlvl,$this->listnum,$this->textbuffer,$this->listoccur[$this->listlvl]);
	        $this->textbuffer = array();
	        $this->listlvl--;
...
{
  error_log("    CloseTag: $tag, $this->pbegin and !$this->divbegin and !$this->tablestart and !$this->buffer_on \n", 3, "/tmp/error.log");
//! @return void
	//Closing tag
  ...
  if($tag=='OUTLINE')
  {
	...
	if( $tag=='A')
	{
	  ...
	  if(!$this->pbegin and !$this->divbegin and !$this->tablestart and !$this->buffer_on)
	  {
       ...
       $this->divwidth = $this->pgwidth - $x + $this->lMargin;
       //Print content
       error_log("     /A: ".print_r($this->textbuffer,true)); //Greg
       //$this->printbuffer($this->textbuffer,true/*is out of a block (e.g. DIV,P etc.)*/);
       $this->SetTextColor(0,0,200);
       $this->SetStyle('U',true);
       $this->Write(5,$this->textbuffer[0][0], $this->textbuffer[0][1], false);
       $this->textbuffer=array();
       //Reset values
       $this->Reset();
    }
    ...
    if(!$this->pbegin and !$this->divbegin and !$this->tablestart)
    {
      if($this->cssbegin)
      {
          ...
          if ($this->cssbegin and ($this->divborder or $this->dash_on or $this->dotted_on or $this->divbgcolor))
          {
              ...
              $this->setX($tempx - 1); //adjust alignment
          }
		      $this->cssbegin=false;
		      $this->backupcss=array();
      }
   ...
   //Reset values
    ...
    if ($this->buffer_on == false) $this->listnum--;//Adjust minor BUG (this happens when there are two </OL> together)
	  if ($this->listlvl == 1) // We are closing the last OL/UL tag
	  {
       ...
       if (!empty(trim($this->textbuffer[0][0]))) {
        $this->listitem[] = array($this->listlvl,$this->listnum,$this->textbuffer,$this->listoccur[$this->listlvl]);
        //error_log('ul or ol ::'.$this->listlvl.' >'.$this->listnum.' ='.$this->textbuffer[0][0].' :'.$this->listoccur[$this->listlvl]);
       }
	     $this->textbuffer = array();
	     $this->listlvl--;
	     $this->printlistbuffer();
...
//! @desc Prints all list-related buffered info
    ...
    //Save x coordinate
    $x = $this->oldx;
    //error_log(var_export($this->listitem, true));
    //error_log(json_encode($this->listitem));
    foreach($this->listitem as $i => $item)
    {
        ...
        if($i == 0){
          $this->x = (5*($lvl-1)) + $x;
            ...
            $this->x = (5*($lvl-1)) + $this->x; //Indent list Greg
        }
...
//! @desc Prepares buffered text to be printed with FlowingBlock()
  ...
  //error_log("printbuffer: outofblock:$outofblock is_table:$is_table  aux:" . var_export($arrayaux, true));
  //error_log(var_export($arrayaux, true));
    ...
    for($i=0;$i < $array_size; $i++)
    {
      ...
      {
        if ($vetor[0] == "\n" || empty(trim($vetor[0]))) //We are reading a <BR> now turned into newline ("\n")
        {
            //Restart Flowing Block
            $this->finishFlowingBlock($outofblock);
        ...
        else 
        {
          ...
          //error_log("Texto: listnum:$this->listnum". json_encode($vetor) . " - " . json_encode($bak_x, true));
          //$this->MultiCell(180,5, $vetor[0], 0, 'L', 0); 
          $this->Write(5, $vetor[0], $this->HREF, $this->listnum);
          
          
          //$this->WriteFlowingBlock( $vetor[0] , $outofblock ); //Greg
        }
      }
...
//! @desc Disable some tags using ',' as separator. Enable all tags calling this function without parameters.
  if ($str == '') //enable all tags
  {
    ...
    $this->enabledtags = "<tt><kbd><samp><option><outline><span><newpage><page_break><s><strike><del><bdo><big><small><address><ins><cite><font><center><sup><sub><input><select><option><textarea><title><form><ol><ul><li><h1><h2><h3><h4><h5><h6><pre><b><u><i><a><img><p><br><strong><em><code><div><th><tr><blockquote><hr><td><tr><table>"; 
  }
  else
  {
    $str = explode(",",$str);
